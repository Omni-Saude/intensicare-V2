...
> **ARQUIVO HISTÓRICO — EXECUTADO E ENCERRADO em 2026-08-20 (commits `1eda4f1` e
> `d1bbde6`).** Este é o texto que viveu em `PROMPT_PROXIMO_ORQUESTRADOR.md` ao
> fim das ondas 3 e 4, e que a sessão das lacunas de escopo executou (com os
> números de baseline já corrigidos em linha durante aquela sessão).
>
> Arquivado sob nome datado porque `PROMPT_PROXIMO_ORQUESTRADOR.md` é ROLANTE —
> mesma disciplina de `ENCARGO_ONDAS_3_E_4_2026-08-18.md`.
>
> **O que foi entregue contra ele** está em `HANDOFF.yaml`, seção
> `lacunas_de_escopo_2026_08_20`. **O encargo vigente** é
> `PROMPT_PROXIMO_ORQUESTRADOR.md`.
>
> Leitura honesta: as ondas 1 e 2 fecharam seis defeitos medidos, `ACH-O3-17`
> (que nenhuma lista previa: o `pnpm verify` não terminava para um humano) e
> três vacuidades. **Quatro dos diagnósticos deste arquivo estavam errados** —
> o código mudara sob eles. O flake de `navegacao.test.tsx` NÃO fechou.
