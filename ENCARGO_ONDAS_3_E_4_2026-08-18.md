...
> **ARQUIVO HISTÓRICO — EXECUTADO E ENCERRADO em 2026-08-18 (commit `f6bcec7`).**
> Este é o texto que viveu em `PROMPT_PROXIMO_ORQUESTRADOR.md` no commit
> `700b13e` e que a sessão das ondas 3 e 4 executou. Foi arquivado aqui, sob
> nome datado, porque aquele arquivo é um documento ROLANTE: ele foi reescrito
> ao fim da sessão com o encargo da sessão SEGUINTE, e sobrescrevê-lo sem
> arquivar apagaria da árvore de trabalho o registro do que foi pedido.
>
> Mesma disciplina aplicada em `HANDOFF.yaml` (seção nova e datada, seções
> anteriores intactas) e nos `docs/**` (emendas datadas, texto anterior
> preservado): **não se reescreve encargo retroativamente.**
>
> **O que foi entregue contra ele** está em `HANDOFF.yaml`, seção
> `ondas_3_e_4_2026_08_18`. **O encargo vigente** é
> `PROMPT_PROXIMO_ORQUESTRADOR.md`, reescrito ao fim desta sessão.
>
> Leitura honesta do resultado: as ondas 3.1/3.2/3.3 e todo o §4 foram
> entregues; a onda 4 (refutação) **derrubou três teses**, achou 25 defeitos —
> um deles P0 — e provou que o §4 deste encargo subdimensionava o `ACH-REV8-3`,
> que estava fechado em duas superfícies de leitura e tinha uma terceira.
