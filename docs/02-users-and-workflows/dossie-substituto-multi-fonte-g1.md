...
version_history:
  - version: 0.1.0-draft
    date: 2026-08-16
    note: >
      Julgada pelo painel AGT-4 (execução 1): VIVE por maioria (3/4) com
      DISSENSO VERIFICADO da lente correção-clínica-fonte-primária (citação
      L-6 infiel). Trilha: docs/15-release-evidence/painel-agt4/trilha/
      2026-08-16-dossie-substituto-multi-fonte-g1-0.1.0-draft.md
  - version: 0.1.1-draft
    date: 2026-08-16
    note: >
      Responde à execução 1 conforme "Direção do integrador" da trilha:
      (a) L-6 corrigida para a forma fiel (re-verificada na fonte nesta
      sessão); (b) narrativa jurídica atualizada ao estado GDEC-0012, sem
      mudança de disposição; (c) VAL-0008/VAL-0011 recategorizados
      ATO-HUMANO com substituto anexado (COBERTO cai para 1 — direção que
      o §4.1 defende); (d) correções de forma (feixe ≥16 anos, branch,
      ponteiros, premissa SPR-G1-8, categoria reservada, citação §20)
...
provenance:
  ...
  commit_sha_or_version: >
    0.1.1-draft redigida sobre 354c4b64cbc7ef58a16683a0611afdf7e24fffcd
    (HEAD de cycle-5/execucao-agentificada); a 0.1.0-draft foi redigida
    sobre 4fba956bc4da1172525f1e0508b242b9a024fcdd e julgada no commit
    d3366f6 (hash do artefato na trilha do painel)
...
evidence_inputs:
  - repo: intensicare-V2
    ...
    commit: 354c4b64cbc7ef58a16683a0611afdf7e24fffcd
    lines_used: "21-64 (AGT-1, AGT-2), 111-121 (AGT-4), 123-137 (honestidade), 170-196 (notas do parecer, complementação e GDEC-0012)"
  - repo: intensicare-V2
    path: docs/00-governance/registers/decision-register.md
    commit: 354c4b64cbc7ef58a16683a0611afdf7e24fffcd
    lines_used: "697-749 (GDEC-0012 íntegra)"
  - repo: intensicare-V2
    path: docs/15-release-evidence/painel-agt4/trilha/2026-08-16-dossie-substituto-multi-fonte-g1-0.1.0-draft.md
    commit: 354c4b64cbc7ef58a16683a0611afdf7e24fffcd
    lines_used: "íntegra (vereditos da execução 1; Direção do integrador itens 1-4)"
  - repo: intensicare-V2
    path: docs/02-users-and-workflows/g1-validation-backlog.md
    commit: 354c4b64cbc7ef58a16683a0611afdf7e24fffcd (idêntico em 4fba956 para os arquivos não alterados pela emenda)
  ...
  - repo: intensicare-V2
    ...
    commit: 354c4b64cbc7ef58a16683a0611afdf7e24fffcd
    lines_used: "305-330 (linhas SPR-G1-8/G1-9 após a emenda GDEC-0012; numeração deslocada pela emenda)"
  - repo: intensicare-V2
...
# Dossiê substituto multi-fonte — Gate G1 (versão 0.1.1-draft)

> **Natureza:** PROPOSAL de AUTOR (painel AGT-4). Cada versão é julgada por
...
>
> **Histórico:** a 0.1.0-draft VIVEU por maioria (3/4) na execução 1 do painel,
> **com dissenso verificado** da lente correção-clínica-fonte-primária (citação
> L-6 infiel — corrigida nesta versão). Trilha imutável:
> `../15-release-evidence/painel-agt4/trilha/2026-08-16-dossie-substituto-multi-fonte-g1-0.1.0-draft.md`.
> Esta 0.1.1-draft executa as correções da "Direção do integrador" e aguarda
> nova execução do painel; a versão apresentável ao MG-G1 é a mais recente com
> VIVE.
...
Este documento É esse dossiê, na sua versão 0.1.1-draft: as quatro pernas estão
...
- **NÃO contém dado retrospectivo real.** A perna (c) é declarada pendente
  (§2.3), nunca simulada — nenhum dado real foi acessado nesta sessão; com o
  parecer OS-16 completo, a liberação de dado real é **por operação**, sob as
  condicionantes decompostas em sprints (GDEC-0012), nunca em bloco.
...
| (c) Dados retrospectivos | **PENDENTE — perna vazia** (SPR-G1-10); parecer OS-16 completo, liberação POR OPERAÇÃO sob condicionantes (GDEC-0012); declarada, jamais simulada (§2.3) |
...
| **L-6** | Difonzo M, "Performance of the Afferent Limb of Rapid Response Systems in Managing Deteriorating Patients: A Systematic Review", Crit Care Res Pract, 2019 | `https://pmc.ncbi.nlm.nih.gov/articles/PMC6874970/` (fetch; números de Kolic, van Galen e Ludikhuize re-verificados por fetch dirigido na redação da 0.1.1) | **SOURCE:** revisão sistemática de 31 estudos (1995–2017, 11 países); *afferent limb failure* = critérios de acionamento documentados sem acionamento do time antes do evento adverso; frequência respiratória mal registrada (14–17% em vários settings); no estudo de Kolic (mesma revisão): **18,9% (70/370) de cálculo incorreto do NEWS** e, separadamente, **25,9% (96/370) de resposta clínica inadequada** — construtos distintos; em outros dois estudos da revisão, apenas **1% (6/477)** e **11% (4/47)** dos escores foram calculados corretamente (van Galen; Ludikhuize); **acionamento tardio em 21–57%** dos pacientes com critérios documentados; atraso associado a mais admissão não planejada em UTI (OR 1,56–1,79), mais mortalidade hospitalar (OR 1,79–2,18) e mortalidade em 30 dias 61,8% vs 41,9% |
...
usuários. **SOURCE** (`INTENSICARE_TECHNICAL_ASSESSMENT.md:56`, citado
verbatim em `workflow-hypotheses.md:39` e `user-research-plan.md:72`): "No
stakeholder interviews or observed ICU workflow studies were performed."
...
- `../05-clinical-safety/legacy-review/clinical-documentation-forms/` e
  ...
  que a V2 NÃO importa por padrão — `../00-governance/legacy-import-policy.md` §1).
...
- **SOURCE (`../00-governance/registers/decision-register.md`, GDEC-0012 +
  notas finais da ata GDEC-0009):** o parecer OS-16 está **COMPLETO**
  (2026-08-16) e a **condição 4 está satisfeita NO PLANO JURÍDICO**, sob as
  **condicionantes POR OPERAÇÃO** do próprio parecer, decompostas em cinco
  sprints do mapa (SPR-G6-6, SPR-G6-7, SPR-G4-7, SPR-G3-13, SPR-G0-4); a 2ª
  revisita da AGT-3 foi conduzida e **APROVADA** (titular + 2º revisor
  clínico); o exercício prático segue **barrado pelas condições 3 e 5-7**.
  Racional registrado: a decomposição "impede que 'parecer recebido' seja lido
  como 'dado real liberado em bloco'".
- **OBSERVED (desta sessão):** nenhum dado retrospectivo foi acessado nesta
  sessão; a perna (c) permanece **vazia** e VAL-0035 permanece pendente. A
  mudança de estado jurídico não muda nenhuma disposição de §4 — muda apenas o
  gargalo declarado: de "parecer pendente" para "condicionantes por operação +
  execução de SPR-G1-10".
- **Disposição desta perna:** os itens que dependem dela ficam
  **PENDENTE-RETROSPECTIVA (OS-16)** em §4 (nome da categoria mantido do
  pacote de tarefa; leia-se "sob as condicionantes por operação do parecer
  OS-16 — GDEC-0012") e serão executados em **SPR-G1-10** ("baseline
  retrospectivo arquivado + declaração de vieses"). Nenhum número
...
- **SOURCE (L-3 + SF-6; L-2 cobre apenas autoria/propósito do NEWS2, não a
  restrição):** NEWS2 é instrumento para ≥16 anos; não recomendado para
  menores de 16 nem gestação. **SOURCE (SF-6):** no
  ...
  fronteira de escore.
- **INFERENCE (de L-3 + SF-6 + IU-06):** a exclusão pediátrica/neonatal não
...
- **SOURCE (repo, WF-06 + SF-3):** conflito e correção são rotina; o legado
  ...
  resolver contradição silenciosamente (proibição do §20 do
  `INTENSICARE_V2_ORCHESTRATOR_PROMPT.md` — never "silently resolve
  contradictions"); mais-recente-vence é resolução silenciosa vestida de
  algoritmo.
...
- **PENDENTE-RETROSPECTIVA (OS-16)** — a evidência substituta virá de dados
  retrospectivos (SPR-G1-10); nome mantido do pacote de tarefa — desde
  GDEC-0012, leia-se: sob as condicionantes POR OPERAÇÃO do parecer OS-16
  completo, nunca "dado real liberado em bloco".
...
- **ATO-HUMANO (fora do objeto do dossiê)** — o item fecha por nomeação,
  ...
  silencioso — **aceito pela arbitragem de DoD do orquestrador na trilha da
  execução 1, item 3**). Insumos substitutos, quando existem, ficam anotados.
- **LACUNA-DE-ENTENDIMENTO (escalada)** — categoria reservada pelo pacote de
  tarefa para item que o autor não compreendesse o suficiente para dispor.
  **Não foi necessária em nenhuma versão até aqui (0 itens)**; declarada no
  vocabulário para que sua ausência na tabela seja escolha visível, não
  omissão.
...
| VAL-0003 | Dono nomeado de privacidade/jurídico | ATO-HUMANO | Parecer OS-16 completo (GDEC-0012) supre a *determinação*; a *nomeação* formal do papel AUTH-PRIVACY-LEGAL segue ato humano — disposição inalterada |
...
| VAL-0006 | Pediátrico em escopo? | ATO-HUMANO | Decisão clínica; substituto anexado: L-3+SF-6 (NEWS2 ≥16 anos; legado sem gating) fundamentam exclusão proposta IU-06, não a decidem |
...
| VAL-0008 | Comportamento fora da população / idade desconhecida | ATO-HUMANO | Recategorizado na 0.1.1 (era COBERTO): o comportamento exigível é decisão AUTH-CLINSAFETY, à moda de VAL-0023; substituto anexado prova só a *necessidade* de enforcement (SF-6 + L-3 + IU-06); compreensão real → RISK-0013 |
...
| VAL-0011 | Consultivo apenas, imposto e não só declarado | ATO-HUMANO | Recategorizado na 0.1.1 (era COBERTO): declarar e impor a fronteira é decisão AUTH-INTENDED-USE+AUTH-CLINSAFETY ratificada no MG-G1; substituto anexado: IU-09/IU-10 (operacionalização testável); leitura real pelo usuário → RISK-0013 |
...
| VAL-0037 | Base legal para adjudicação de desfecho e subgrupo | ATO-HUMANO | Parecer OS-16 completo dá a base NO PLANO JURÍDICO; as condicionantes POR OPERAÇÃO (GDEC-0012: 5 sprints, incl. RIPD e instrumento art. 39) são atos humanos por operação — disposição inalterada |
...
|---|:--:|---|
| COBERTO-POR-SUBSTITUTO | 1 | VAL-0026 |
...
| ATO-HUMANO (fora do objeto do dossiê) | 14 | VAL-0001, 0002, 0003, 0004, 0006, 0007, 0008, 0009, 0010, 0011, 0023, 0030, 0036, 0037 |
...
**INFERENCE (honestidade da contagem):** na 0.1.1, apenas **1** item é
genuinamente *coberto* por substituto — VAL-0026, o único em que a pergunta do
backlog era respondível por literatura+forense+documento sem resíduo decisório
dominante. VAL-0008 e VAL-0011 foram recategorizados para ATO-HUMANO em
resposta à execução 1 do painel (recomendação das lentes revisor e
fail-closed, acolhida pelo integrador): a cobertura que tinham era da parte
*evidencial*; decisão e enforcement sempre foram humanos. A queda de 3→1 na
coluna COBERTO é a direção que este dossiê defende: inflar "coberto" para
parecer melhor seria o anti-padrão que o programa proíbe (contagem sai do
gate, não de meta). Os 20 itens sob RISCO-ACEITO-RISK-0013 são exatamente o
conteúdo material do risco que o titular aceitou. Os 14 ATO-HUMANO não são
dispostos *por* este dossiê — são carregados à vista, com seus fóruns de
fechamento nomeados, para que nenhum feche por silêncio.
...
6. **Risco líquido (SOURCE, RISK-0013):** "desenhar fluxo e alarmística para um
   ...
   missão.
7. **Limitações próprias desta versão 0.1.1-draft (OBSERVED, desta sessão):**
   (i) perna (c) vazia — nenhum número retrospectivo; com o parecer OS-16
   completo, o gargalo deixou de ser o parecer e passou a ser as
   condicionantes por operação (GDEC-0012) e a execução de SPR-G1-10; (ii)
...
1. **A aprovação humana permanece.** SOURCE (mapa, nota do marco MG-G1):
   ...
   **SPR-G1-8** do mapa: "AGT-1 trocou a evidência do gate, não o aprovador
   humano".
...
3. **BLK-0008 permanece OPEN** até o MG-G1; nenhuma linha deste documento o
   fecha.
4. **A contagem dura permanece:** 0 vias acionáveis; o bloqueio dominante
   segue sendo de dados (G3/AMH) — SOURCE (ata, "Honestidade obrigatória",
   item 1). Com o parecer OS-16 **completo** (GDEC-0012), a frase da ata
   "OS-16 é o único prazo externo" **já não descreve o estado**: o que resta
   são as condicionantes por operação (5 sprints: SPR-G6-6/G6-7/G4-7/G3-13/
   G0-4) e as condições 3 e 5-7 da AGT-3 — "parecer recebido ≠ dado real
   liberado em bloco".
...
- `../14-devsecops-and-delivery/mapa-de-projeto-ate-producao.md` — SPR-G1-9 (este sprint), SPR-G1-8 (MG-G1), SPR-G1-10 (perna retrospectiva); sprints das condicionantes do parecer (GDEC-0012).
- `../15-release-evidence/painel-agt4/trilha/2026-08-16-dossie-substituto-multi-fonte-g1-0.1.0-draft.md` — trilha da execução 1 (VIVE 3/4 com dissenso verificado; Direção do integrador que esta versão executa).
