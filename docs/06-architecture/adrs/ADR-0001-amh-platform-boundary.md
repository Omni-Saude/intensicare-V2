...
| E12 | SOURCE | No AMH-owner contact has been established. `AUTH-AMH-OWNER` is `UNASSIGNED — VALIDATION REQUIRED` and the governance registers carry open blockers for it. | `docs/00-governance/authority-model.md` §2 | high |
...
| E15 | SOURCE | Nothing may be written to the AMH repository; creating an AMH×IntensiCare contract requires separate AMH-owner authority and review. | prompt §7.5; `compatibility-finding.md` §6 | high |
...
|---|---|---|---|---|
| C1 | A named, reachable AMH owner exists and has approved the responsibility boundary in writing. | `AUTH-AMH-OWNER` | Recorded approval naming the boundary, the responsibilities on each side, and the escalation route. | **OPEN** — no AMH-side contact established (E12) |
| C2 | A named V2 data-platform decision owner exists. | Gate G0 | `authority-model.md` row populated with a human. | **OPEN** |
