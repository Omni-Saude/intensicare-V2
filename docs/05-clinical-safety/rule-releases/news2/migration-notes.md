...
statement: >
  Rule-local summary of what RULE-NEWS2 0.2.0 supersedes, retains as re-derived
  published content, and rejects from the legacy V1 implementation, citing only the
  cycle-1 forensic review records. REVISADO CLINICAMENTE 2026-08-15 (GDEC-0007) —
  decisões incorporadas; aprovação formal pendente do mecanismo de bundle assinado
  (ADR-0007).
provenance:
...
# RULE-NEWS2 0.2.0 — migration notes (rule-local)
...
as code." RULE-NEWS2 0.2.0 is the TRANSFORM output: a re-derivation from the RCP 2017
...
| Float-rounding guard **concept** (compare at chart resolution) | `specification.md` §4.1, REFINEd; tie direction DECIDIDA (GDEC-0007 N-7 (a)): meio-passo exato arredonda para a banda mais anormal | `news2-review.md` §6 RETAIN-as-concept. |
...
propose NEWS2 as the early-warning-score **family representative**.
**DECISÃO (GDEC-0007, 2026-08-15, folha A25-2 (a)):** assumir **SUPERSEDE** do MEWS
pelo NEWS2 enquanto G2 pende; registro MEWS **dormente** (sobreposição total de
insumos, citação mais fraca, dois EWS = alarmes correlatos). A verificação da Tabela 1
do Subbe é moot sob esta decisão (A25-3); se MEWS voltar, é tarefa do metodologista de
evidência. `supersedes` fields are updated at the portfolio level — not here
preemptively; this file remains the rule-local record.
