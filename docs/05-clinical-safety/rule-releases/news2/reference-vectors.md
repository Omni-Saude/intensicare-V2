...
title: NEWS2 clinical reference vectors — CRV set for RULE-NEWS2 0.1.0
label: PROPOSAL
statement: >
  Clinical reference vectors for RULE-NEWS2 0.1.0 per the clinical-reference-vector
  standard. Every vector is PROPOSAL — pending independent clinical review; authorship
  independence is NOT satisfied (vector author = specification author), so no vector
  here may be cited as release evidence.
provenance:
  ...
  commit_sha_or_version: "RCP NEWS2 (2017) issuer PDF; RULE-NEWS2 specification.md 0.1.0 (same directory, same authoring session)"
...
# RULE-NEWS2 reference vectors (CRV set) — 0.1.0

> **Every vector below is PROPOSAL — pending independent clinical review (reviewer:
> rodaquino-OMNI). Author ≠ approver applies to vectors too.**
...
- **ID block (PROPOSAL):** `CRV-0101`–`CRV-0189`, a provisional block claimed to avoid
  collision with concurrent specialists; no central CRV catalog exists yet. Final IDs
  are assigned at catalog registration; the block claim is itself flagged for the
  traceability steward. `CRV-0000` remains reserved-illustrative per the standard.
...
- **Reason-taxonomy note:** the CRV standard routes source-`quarantined` under
  ...
  (`quarantined_input`). CRV-0118 records this tension; resolution belongs to the
...
|---|---|---|---|---|---|
| CRV-0101 | none — baseline as stated | all params 0; Total 0; low | `valid` | false | `criteria_not_met` |
...
|---|---|---|---|---|---|---|
| CRV-0102 | **ALL seven inputs absent** (`value.present: false` on every entry) | **no total — never 0** (HAZ-0005 standing regression vector; legacy returned total 0, risk "low") | `not_evaluated` | false | `insufficient_data` (per-param `missing_required_input:*` ×7) | `all-inputs-missing` |
| CRV-0103 | RR absent | no total | `not_evaluated` | false | `insufficient_data` (`missing_required_input:rr`) | `single-input-missing` |
| CRV-0104 | SpO2 absent | no total | `not_evaluated` | false | `insufficient_data` (`missing_required_input:spo2`) | `single-input-missing` |
| CRV-0105 | supplemental-O2 status absent (SpO2 present) | no total — **unknown is never "air"** | `not_evaluated` | false | `insufficient_data` (`missing_required_input:o2_status`) | `single-input-missing` |
| CRV-0106 | SBP absent | no total | `not_evaluated` | false | `insufficient_data` (`missing_required_input:sbp`) | `single-input-missing` |
| CRV-0107 | pulse absent | no total | `not_evaluated` | false | `insufficient_data` (`missing_required_input:pulse`) | `single-input-missing` |
| CRV-0108 | consciousness absent | no total — absent ACVPU is never 0 points | `not_evaluated` | false | `insufficient_data` (`missing_required_input:consciousness`) | `single-input-missing` |
| CRV-0109 | temperature absent | no total | `not_evaluated` | false | `insufficient_data` (`missing_required_input:temperature`) | `single-input-missing` |
| CRV-0110 | RR observed 90 min before evaluation (window 60 min, expiry 8 h) | no fresh total | `not_evaluated` | false | `stale_data` (`stale_input:rr`) | `freshness-window-edge` |
| CRV-0111 | RR observed exactly 60 min before evaluation (at window edge, inclusive) | Total 0; low | `valid` | false | `criteria_not_met` | `freshness-window-edge` |
| CRV-0112 | RR observed 9 h before evaluation (past 8 h expiry) | no total | `not_evaluated` | false | `stale_data` (`expired_input:rr`) | `freshness-window-edge` |
| CRV-0113 | two simultaneous SBP values, 80 and 150, same clinical time, no resolution | no total | `invalid` | false | `invalid_data` (`conflicting_sources:sbp`) | `conflicting-sources` |
| CRV-0114 | age/DOB absent | no total — **never assume adult** (VAL-0006/0007) | `not_evaluated` | false | `insufficient_data` (`unknown_age`) | `population-exclusion-boundary` |
| CRV-0115 | age 17 (verified) | no total under proposed ≥18 gate; RCP-permitted band — reviewer Q1 | `not_evaluated` | false | `out_of_population_scope` | `population-exclusion-boundary` |
| CRV-0116 | consciousness token `SEDATED` (schema-unmapped) | **`invalid`, never 0** (legacy D-7: HL7 'C'→None→0; D-8: any-non-A→3 — both rejected) | `invalid` | false | `invalid_data` (`unmappable_code:consciousness`) | `conflicting-sources`-adjacent; recorded as edge |
| CRV-0117 | SpO2 = 150 % | outside plausible range 40–100 | `invalid` | false | `invalid_data` (`implausible_value:spo2`) | `threshold-exact-match` (plausibility bound) |
| CRV-0118 | RR present and in-window but `source_data_quality: quarantined` | quarantined input never contributes | `not_evaluated` | false | `insufficient_data` (`quarantined_input:rr`) — taxonomy tension recorded in §1 | `source-quality-vs-evaluation-status-independence` |

### 2.3 SpO2 scale governance (5 vectors)

| ID | Scenario (delta vs baseline) | Expected | Status | fires | reason | boundary class |
|---|---|---|---|---|---|---|
| CRV-0119 | `spo2_scale_assignment: scale2` (documented order); SpO2 **70 %** on **oxygen** | SpO2 = **3** (low Scale-2 bands apply regardless of O2 — legacy D-1 scored this 0); O2 = 2; Total 5; medium + red param | `valid` | true | fire: red param + total ≥5 | `threshold-exact-match` |
| CRV-0120 | scale2 order; SpO2 **90 %** on oxygen (at BTS 88–92 target) | SpO2 = **0** (legacy D-2 scored 1; legacy production Scale-1 path scored 2–3); O2 = 2; Total 2; low | `valid` | false | `criteria_not_met` | `threshold-exact-match` |
| CRV-0121 | **NO scale2 order**; SpO2 91 % on oxygen | **Scale 1 stays** (supplemental O2 never flips scale — D-3/D-4 regression): SpO2 = 3; O2 = 2; Total 5; medium + red param | `valid` | true | fire: red param + total ≥5 | `multi-criteria-combination` |
| CRV-0122 | no scale2 order (absent governed input); SpO2 95 % on air | Scale 1 default per RCP Rec 27: SpO2 = 1; Total 1; low | `valid` | false | `criteria_not_met` | `single-input-missing` (governed input absent ⇒ safe default, not not_evaluated) |
| CRV-0123 | scale2 order; SpO2 94 %; supplemental-O2 status **absent** | ≥93 region needs air/oxygen to band; O2 status is required anyway | `not_evaluated` | false | `insufficient_data` (`missing_required_input:o2_status`) | `single-input-missing` |
...
|---|---|---|---|---|---|---|
| CRV-0124 | 83, air | 3 | 3 | low_medium (red) | `valid` | true |
| CRV-0125 | 84, air | 2 | 2 | low | `valid` | false |
| CRV-0126 | 85, air | 2 | 2 | low | `valid` | false |
| CRV-0127 | 86, air | 1 | 1 | low | `valid` | false |
| CRV-0128 | 87, air | 1 | 1 | low | `valid` | false |
| CRV-0129 | 88, air | 0 | 0 | low | `valid` | false |
| CRV-0130 | 92, air | 0 | 0 | low | `valid` | false |
| CRV-0131 | 93, air | 0 | 0 | low | `valid` | false |
| CRV-0132 | 92, oxygen | 0 | 2 | low | `valid` | false |
| CRV-0133 | 93, oxygen | 1 | 3 | low | `valid` | false |
| CRV-0134 | 94, oxygen | 1 | 3 | low | `valid` | false |
| CRV-0135 | 95, oxygen | 2 | 4 | low | `valid` | false |
| CRV-0136 | 96, oxygen | 2 | 4 | low | `valid` | false |
| CRV-0137 | 97, oxygen | 3 | 5 | medium + red | `valid` | true |
...
| CRV-0143 | RR | 21 | 2 |
| CRV-0144 | RR | 24 | 2 |
| CRV-0145 | RR | 25 | 3 (red) |
| CRV-0146 | SpO2 Scale 1 | 91 | 3 (red) |
| CRV-0147 | SpO2 Scale 1 | 92 | 2 |
| CRV-0148 | SpO2 Scale 1 | 93 | 2 |
| CRV-0149 | SpO2 Scale 1 | 94 | 1 |
| CRV-0150 | SpO2 Scale 1 | 95 | 1 |
| CRV-0151 | SpO2 Scale 1 | 96 | 0 |
| CRV-0152 | O2 status | oxygen (SpO2 97, Scale 1) | O2 = 2; Total 2 |
| CRV-0153 | O2 status | air (documented) | O2 = 0; Total 0 |
| CRV-0154 | SBP | 90 | 3 (red) |
| CRV-0155 | SBP | 91 | 2 |
| CRV-0156 | SBP | 100 | 2 |
| CRV-0157 | SBP | 101 | 1 |
| CRV-0158 | SBP | 110 | 1 |
| CRV-0159 | SBP | 111 | 0 |
| CRV-0160 | SBP | 219 | 0 |
| CRV-0161 | SBP | 220 | 3 (red) |
| CRV-0162 | Pulse | 40 | 3 (red) |
| CRV-0163 | Pulse | 41 | 1 |
| CRV-0164 | Pulse | 50 | 1 |
| CRV-0165 | Pulse | 51 | 0 |
| CRV-0166 | Pulse | 90 | 0 |
| CRV-0167 | Pulse | 91 | 1 |
| CRV-0168 | Pulse | 110 | 1 |
| CRV-0169 | Pulse | 111 | 2 |
| CRV-0170 | Pulse | 130 | 2 |
| CRV-0171 | Pulse | 131 | 3 (red) |
| CRV-0172 | Consciousness | A | 0 |
| CRV-0173 | Consciousness | C (new confusion) | 3 (red) — RCP Recs 29–30 |
| CRV-0174 | Consciousness | V | 3 (red) |
| CRV-0175 | Consciousness | P | 3 (red) |
| CRV-0176 | Consciousness | U | 3 (red) |
| CRV-0177 | Temperature | 35.0 | 3 (red) |
| CRV-0178 | Temperature | 35.1 | 1 |
| CRV-0179 | Temperature | 36.0 | 1 |
| CRV-0180 | Temperature | 36.1 | 0 |
| CRV-0181 | Temperature | 38.0 | 0 |
| CRV-0182 | Temperature | 38.1 | 1 |
| CRV-0183 | Temperature | 39.0 | 1 |
| CRV-0184 | Temperature | 39.1 | 2 |

### 2.6 Aggregate thresholds and tiers (5 vectors)

| ID | Scenario (deltas) | Component scores | Total | Tier | Status | fires | reason |
|---|---|---|---|---|---|---|---|
| CRV-0185 | RR 21; pulse 111 | 2 + 2, no single 3 | 4 | low | `valid` | false | `criteria_not_met` — 4/5 boundary, below |
| CRV-0186 | RR 21; pulse 111; temp 35.6 | 2 + 2 + 1 | 5 | medium | `valid` | true | fire: total ≥ 5 — 4/5 boundary, at |
| CRV-0187 | RR 21; pulse 111; temp 35.6; SBP 105 | 2 + 2 + 1 + 1 | 6 | medium | `valid` | true | fire: total ≥ 5 — 6/7 boundary, below |
| CRV-0188 | on oxygen (SpO2 96, Scale 1 = 0); RR 21; pulse 111; temp 35.6 | 2 + 2 + 2 + 1 | 7 | high | `valid` | true | fire: total ≥ 7 — 6/7 boundary, at |
| CRV-0189 | temp 35.0 only | single 3, aggregate 3 | 3 | **low_medium (red)** | `valid` | true | fire: single red param with low aggregate — RCP Chart 2 Low–medium tier |
...
### CRV-0102 — all inputs missing (HAZ-0005 standing regression)

```yaml
vector_id: CRV-0102
title: "All seven NEWS2 inputs absent yields not_evaluated - never total 0"
pathway_id: RULE-NEWS2
rule_version: {bundle: news2, version: 0.1.0, content_hash: UNASSIGNED-precursor, status: draft}
scenario_class: edge
...
### CRV-0119 — Scale 2, SpO2 70 % on oxygen scores 3, never 0 (legacy D-1 defect case)
...
### CRV-0116 — unmapped consciousness token is invalid, never 0 and never 3
...
| Category | Vectors |
|---|---|
| Typical/normal | 1 (CRV-0101) |
| Missing data, staleness, integrity, population | 17 (CRV-0102–0118) |
| SpO2 scale governance | 5 (CRV-0119–0123) |
| SpO2 Scale 2 band boundaries | 14 (CRV-0124–0137) |
| Scale 1 + remaining parameter band boundaries | 47 (CRV-0138–0184) |
| Aggregate thresholds and tiers | 5 (CRV-0185–0189) |
| **Total** | **89** |

## 5. Open items for this vector set

1. Independent re-authoring or independent clinical review of every expected outcome
   (section 0) — blocking for any progression beyond DRAFT.
2. CRV ID block registration in a central catalog (none exists yet).
3. Resolution of the quarantined-input reason-taxonomy tension (§1, CRV-0118).
4. Freshness-window vectors (CRV-0110–0112) inherit proposed window numbers from
   `specification.md` §2.2 — they must be re-derived if the reviewer changes the
   windows.
5. Rounding-tie vectors are deliberately absent pending reviewer Q7 (tie direction);
   add ± half-step vectors for every numeric boundary once decided.
